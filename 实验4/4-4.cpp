/*在上题中补充完成求二叉树深度算法，并在主函数中补充相应的调用验证正确性。
文件名：*_实验4-4.cpp
...
int leafnum(BiTree p) { //求二叉树叶子结点数
    ...
    int n0;
    if (p == NULL) n0 = 0; //空树
    else if (p->lchild == NULL && p->rchild == NULL) //无左右子 即为叶子结点
        n0 = 1;
    else
...
int depth(BiTree p) {  //求二叉树深度
    ...
    int d1, d2;
    if (p == NULL) return 0; //空树当然0深度
    else {
        d1 = 1 + depth(p->lchild); //不便解释
        d2 = 1 + depth(p->rchild); //不便解释
        return d1 > d2 ? d1 : d2;
    }
}
